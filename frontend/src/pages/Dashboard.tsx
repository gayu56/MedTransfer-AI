...
export default function Dashboard() {
  ...
  const stats = [
    { label: 'Active Transfers', value: analytics?.active_transfers ?? 0, icon: Activity, color: 'text-primary-600', bg: 'bg-primary-50' },
    { label: 'Completed', value: analytics?.completed_transfers ?? 0, icon: CheckCircle, color: 'text-emerald-600', bg: 'bg-emerald-50' },
    { label: 'Total', value: analytics?.total_transfers ?? 0, icon: Clock, color: 'text-slate-600', bg: 'bg-slate-100' },
    { label: 'Emergent', value: analytics?.by_urgency?.emergent ?? 0, icon: AlertTriangle, color: 'text-rose-600', bg: 'bg-rose-50' },
  ]
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Transfer Dashboard</h1>
      ...
      <div className="bg-white rounded-xl border border-slate-200">
        <div className="px-6 py-4 border-b border-slate-200">
          <h2 className="text-lg font-semibold text-slate-900">Recent Transfers</h2>
        </div>
        {transfers.length === 0 ? (
          <div className="p-12 text-center">
            <Activity className="w-12 h-12 text-slate-300 mx-auto mb-3" />
            <p className="text-slate-500 text-sm">No transfers yet. Start by creating a new transfer.</p>
            <Link to="/transfers/new" className="text-primary-600 text-sm font-medium hover:underline mt-2 inline-block">
              Create New Transfer →
            </Link>
          </div>
        ) : (
